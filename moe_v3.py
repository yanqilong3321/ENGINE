    ...
    if config.sampler =='rw':
        ...
        if config.dataset in ['ogbn-arxiv', 'arxiv_2023', 'photo'] and os.path.exists(train_path) and os.path.exists(val_path) and os.path.exists(test_path):
            ...
            if config.dataset in ['ogbn-arxiv', 'arxiv_2023', 'photo']:
                os.makedirs(f'../subgraphs/{config.dataset}/khop-1', exists_ok = True)
    ...
    for epoch in tqdm(range(config.epochs)):
        for data in train_loader:
            ...
            for i, m  in enumerate(model_list):
                ...
                last = out
                config.encoder == 'MOE' and gates_list.append(gates)  # 收集门控输出
                ...
                print(f"relevant_logits shape: {relevant_logits.shape},{hid_logits.shape}")
                #print(f"relevant_labels min: {relevant_labels.min()}, max: {relevant_labels.max()}")
                print(f"relevant_labels shape: {relevant_labels.shape}")
                exit()
...
if __name__ == '__main__':
    ...
    for i, seed in enumerate(config.seeds):
        ...
        # 输出最终的AA和AF
        final_AA = sum(AA_list) / len(AA_list)
        final_AF = sum(AF_list) / len(AF_list) if AF_list else 0.0
